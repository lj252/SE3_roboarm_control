"""
UR12eHW — UR12e 具体硬件实现
=============================

基于通用 URHW 类的 UR12e 专用封装。
...
class UR12eHW(URHW):
    """UR12e 硬件接口 (URHW 的 UR12e 封装)。

    所有 RTDE 逻辑继承自 URHW 类。
    此处仅指定 UR12e 特有的力矩限幅和关节名称。

    :param str ip:            UR12e 控制箱 IP 地址
    ...
    def __init__(
        ...
        super().__init__(
            ip=ip,
            torque_limits=_UR12E_TORQUE_LIMITS.copy(),
            joint_names=_UR12E_JOINT_NAMES.copy(),
            robot_name="UR12e",
            dt=dt,
            timeout=timeout,
            verbose=verbose,
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print(f"UR12eHW 自检 — 连接 {ip}")
    print("=" * 60)
    print(f"\n⚠️  请确保 UR12e 已开机且教示器处于远程控制模式 (Remote Control)")
    print(f"   IP: {ip}")
    print(f"   按 Ctrl+C 终止\n")

    try:
        with UR12eHW(ip=ip, verbose=True) as robot:
            q, dq = robot.get_joint_states()
            print(f"\n  关节位置: {np.round(q, 4)}")
